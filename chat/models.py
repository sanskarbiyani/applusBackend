...
class UserStatus(models.Model):
    user = models.OneToOneField(
        NewUser, on_delete=models.CASCADE, primary_key=True)
...
class Thread(models.Model):
    user1 = models.ForeignKey(
        'users.NewUser', on_delete=models.CASCADE, related_name='user1', null=True)
    user2 = models.ForeignKey(
        'users.NewUser', on_delete=models.CASCADE, related_name='user2', null=True)
    updated_at = models.DateTimeField(auto_now_add=True, null=True)
    # unread_count = models.IntegerField(default=0, null=False)

    def __str__(self) -> str:
        return f'Thread Between Users {self.user1} and {self.user2}  --> {self.updated_at}'
...
class Messages(models.Model):
    ...
    msg_type = models.CharField(
        max_length=2, choices=MESSAGE_TYPES, default='1')
    message = models.CharField(max_length=1200)
    fileId = models.ForeignKey(
        'chat.UploadedFiles', on_delete=models.CASCADE, null=True)
...
class UploadedFiles(models.Model):
    ...
    sender = models.ForeignKey(
        'users.NewUser', on_delete=models.CASCADE, default='')
