...
class ModelSchema(models.Model):
    ...
    def get_db_table(self):
        return self._factory._model_meta()
...
class FieldSchema(models.Model):
    ...
    def validate(self):
        print("From the model.py",self.columns)
        if self._initial_null and not self.null:
            print("self._initial_null:-",self._initial_null, "and", self.null)
    ...
    def get_options(self):
        ...
        #     options['input_formats'] = settings.DATE_INPUT_FORMATS
        
        if self.data_type == 'file':
            
            options['upload_to'] = 'user_{0}/'.format(self.model_schema.name)
        return options
