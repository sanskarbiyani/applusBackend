...
class Allgroups(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        for grp in Group.objects.all():
            data[grp.name] =  List_group_link.objects.filter(group=grp).values('group__name','list__modelname','list__color','list__icon')
        print(data)
    ...
    def post(self,request):
        ...
        User_group_link.objects.create(user=request.user,group=obj,permission='edit')
...
class GeneralViewSet(viewsets.ModelViewSet):
    ...
    def model(self):
        ...
        #         print(model)
        model = apps.get_model(app_label=str('ddmapp'), model_name=str(self.kwargs['model_name']).replace(" ",""))
        return model
    ...
    def get_queryset(self):
        ...
        self.check_object_permissions(self.request,{'list':self.kwargs['model_name'],'group':self.kwargs['group_name']})
        return obj        
 ...
 #       # LogEntries.objects.create(user = request.user, contenttype = "{} created new Field {} -  in {} group.".format(request.user,(request.data)['modelname'],(request.data)['group']), actionflag = "Create")
    ...
    def upadteSingleCell(self, request, **kwargs):
        try:
            ...
            # print(request.data)
            reg_serializer1 = self.get_serializer(instance,data=request.data,partial=True)
            print("Hellp")
    ...
    def getinfo(self, request, **kwargs):
        ...
        try:
            self.check_object_permissions(self.request,{'list':self.kwargs['model_name'],'group':self.kwargs['group_name']})
            validation = User_group_link.objects.get(user=self.request.user.id,group__name=self.kwargs['group_name'])
            ...
            try:
                obj = List_Database.objects.get(created_by=self.request.user.id,modelname=self.kwargs['model_name'])
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        LogEntries.objects.create(user = request.user, contenttype = "{} Delete record id = '{}' from {} list.".format(request.user,obj.id,self.kwargs.get('model_name')), actionflag = "Delete",list=List_Database.objects.get(modelname=self.kwargs.get('model_name')) )
...
class DeleteModel(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        objContent = ContentType.objects.get(model=item.lower().replace(" ", ""))
        ...
        LogEntries.objects.create(user = request.user, contenttype = "{} Delete  List ( {} ) ".format(request.user,item), actionflag = "Delete")
        reload(import_module(settings.ROOT_URLCONF))
        clear_url_caches()
...
class DeleteField(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            #step3 :- Drop Model From SQLite Database
            LogEntries.objects.create(user = request.user, contenttype = "{} Delete  Field ( {} ) in {} List.".format(request.user,self.kwargs.get('id'),self.kwargs['model_name']), actionflag = "Create",list= List_Database.objects.get(modelname=self.kwargs['model_name']))
...
class EditModelname(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            print(instance)
            print(request.data)
            reg_serializer1 = List_Database_Serializer(instance,data=request.data,partial=True)
            print(reg_serializer1.is_valid())
            print(reg_serializer1.errors)
            reg_serializer1.save()
            print("64-done")
            try:
                if request.data['modelname']: 
                    ...
                    print(instance)
                    unregister(instance)
                    reg_serializer = ModelSchemasSerializer(instance, data=data, partial=True)
...
class AddNewListToGroup(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        obj = List_group_link.objects.get(list=str(List_Database.objects.get(modelname=str(list1).replace("_", " ")).id),group=str(Group.objects.get(name=list2).id))
        self.check_object_permissions(self.request,obj)
        return obj
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            LogEntries.objects.create(user = request.user, contenttype = "{} Share Lists in {} group.".format(request.user,",".join(newuser)), actionflag = "Create",list=List_Database.objects.get(modelname=request.data['list']))
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            objs = self.get_object()
            self.perform_destroy(objs)
            LogEntries.objects.create(user = request.user, contenttype = "{} Remove  List ( {} ) from {} group.".format(request.user,list1,list2), actionflag = "Delete",list=List_Database.objects.get(modelname=list1))
            #step3 :- Drop Model From SQLite Database
            return Response("Delete Sucessfully",status=status.HTTP_200_OK)
        except Exception as e:
            return Response(data={
...
class LogEntriesDisplay(viewsets.ModelViewSet):
    ...
    def get_object(self):
        item = self.kwargs.get('pk')
        obj = LogEntries.objects.filter(list=List_Database.objects.get(modelname=item))
...
class RequestFormEntryEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if True:
            ...
            relativeLink = reverse('ddmapp_api:form-reset-confirm', kwargs={'uidb64': uidb64, 'token': token, 'entry_id':1,'modelname':'ListOne'})
...
class SetNewFormDataAPIView(viewsets.ModelViewSet):
    ...
    @property
    def model(self):
        model = apps.get_model(app_label=str('ddmapp'), model_name=str(self.kwargs['model_name']))
        return model
